	...
	for (i=0; i < connectionRequestList.Size(); i++)
	{
		if (connectionRequestList[i]->receiverGuid==packet->guid)
		{
			...
			if (connectionRequestList[i]->advertisedAddress==UNASSIGNED_SYSTEM_ADDRESS)
			{
				...
				rakPeer->CloseConnection(connectionRequestList[i]->lastAddressAttempted, false, 0);
				rakPeer->Connect(packet->systemAddress.ToString(false), packet->systemAddress.port, connectionRequestList[i]->passwordData, connectionRequestList[i]->passwordDataLength);
